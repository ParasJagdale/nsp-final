            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className={`form-input ${aadharError ? 'border-red-500' : ''}`}
                    maxLength={12}
                    value={form.aadhar}
                    onChange={e => {
                  ...
                  />
                  {aadharError && (
                    <div className="text-xs text-red-600 mt-1">{aadharError}</div>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    className={`form-input ${communityError ? 'border-red-500' : ''}`}
                    placeholder="SC / ST / OBC / General"
                    value={form.community}
                    onChange={e => {
                      const val = e.target.value.trim();
                      set('community', val);
                      if (val && !allowedCommunities.includes(val)) {
                        setCommunityError('Please enter SC, ST, OBC, or General only');
                      } else {
                        setCommunityError('');
                      }
                    }}
                  ...
                  />
                  {communityError && (
                    <div className="text-xs text-red-600 mt-1">{communityError}</div>
                  )}
                </div>
                <div>
                  <label className="form-label">Father's Name *</label>
                  <input
                    className={`form-input ${fatherNameError ? 'border-red-500' : ''}`}
                    value={form.fatherName}
                    onChange={e => {
                      const val = e.target.value;
                      set('fatherName', val);
                      if (val && !/^[a-zA-Z ]+$/.test(val)) {
                        setFatherNameError('Father\'s Name must contain alphabets and spaces only');
                      } else {
                        setFatherNameError('');
                      }
                    }}
                  ...
                  />
                  {fatherNameError && (
                    <div className="text-xs text-red-600 mt-1">{fatherNameError}</div>
                  )}
                </div>
                <div>
                  <label className="form-label">Mother's Name *</label>
                  <input
                    className={`form-input ${motherNameError ? 'border-red-500' : ''}`}
                    value={form.motherName}
                    onChange={e => {
                      const val = e.target.value;
                      set('motherName', val);
                      if (val && !/^[a-zA-Z ]+$/.test(val)) {
                        setMotherNameError('Mother\'s Name must contain alphabets and spaces only');
                      } else {
                        setMotherNameError('');
                      }
                    }}
                  ...
                  />
                  {motherNameError && (
                    <div className="text-xs text-red-600 mt-1">{motherNameError}</div>
                  )}
                </div>
